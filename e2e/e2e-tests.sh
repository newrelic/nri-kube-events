...
function create_cluster() {
    ...
    docker buildx build --load . --tag e2e/nri-kube-events:e2e --quiet > /dev/null

    echo "🔄 Loading image into cluster"
    minikube image load e2e/nri-kube-events:e2e --profile ${CLUSTER_NAME} > /dev/null

    echo "🔄 Adding Helm repositories"
    helm repo add nri-kube-events https://newrelic.github.io/nri-kube-events > /dev/null
